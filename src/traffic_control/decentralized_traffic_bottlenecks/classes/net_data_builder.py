...
if __name__ == "__main__":
    import sys
    # Usage: python net_data_builder.py <input.net.xml> <output.json>
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} <net_file> <out_json>")
        sys.exit(1)
    nd = NetworkData(sys.argv[1], sys.argv[2])
    nd.to_json_file(sys.argv[2])
