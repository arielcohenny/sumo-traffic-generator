...
def verify_convert_zones_to_projected_coordinates(zones_file: str, network_file: str) -> None:
    ...
    try:
                ...
                for coord_pair in shape.split(' '):
                    if ',' in coord_pair:
                        try:
                            ...
                            logger.warning(
                                f"Invalid coordinate format: {coord_pair}")
                            continue

        if not zone_coords:
            raise ValidationError(
                "No valid zone coordinates found in zones file")
        ...
        if (abs(net_bounds[0]) < 180 and abs(net_bounds[1]) < 180 and
            ...
            logger.warning(
                "Network bounds appear to be in geographic coordinates - this may indicate transformation issues")

        if (abs(zone_bounds[0]) < 180 and abs(zone_bounds[1]) < 180 and
                abs(zone_bounds[2]) < 180 and abs(zone_bounds[3]) < 180):
            logger.warning(
                "Zone bounds appear to be in geographic coordinates - coordinate transformation may have failed")
        ...
        if net_width <= 0 or net_height <= 0:
            raise ValidationError(
                f"Invalid network bounds: width={net_width}, height={net_height}")

        if zone_width <= 0 or zone_height <= 0:
            raise ValidationError(
                f"Invalid zone bounds: width={zone_width}, height={zone_height}")
        ...
        logger.info(
            f"Zone coverage - X: {coverage_x:.2f}, Y: {coverage_y:.2f}")
        ...
        logger.info(
            f"Zone coverage validation passed - X: {coverage_x:.2f}, Y: {coverage_y:.2f}")
...
def _validate_coordinate_transformation_quality(zone_coords: List[Tuple[float, float]],
    ...
    for x, y in zone_coords:
        if abs(x) > 1e8 or abs(y) > 1e8:
            logger.warning(
                f"Coordinate appears unreasonably large: ({x}, {y}) - possible transformation error")

        if x == 0 and y == 0:
            logger.warning(
                "Found (0,0) coordinate - possible failed transformation")
    ...
    # More than 10% identical coordinates is suspicious
    duplicate_threshold = len(zone_coords) * 0.1
    max_duplicates = max(coord_counts.values())

    if max_duplicates > duplicate_threshold:
        logger.warning(
            f"High number of duplicate coordinates ({max_duplicates}) - possible transformation issue")
    ...
    center_distance = ((zone_center_x - net_center_x)**2 +
                       (zone_center_y - net_center_y)**2)**0.5
    net_diagonal = ((net_bounds[2] - net_bounds[0])
                    ** 2 + (net_bounds[3] - net_bounds[1])**2)**0.5

    if center_distance > net_diagonal:
        logger.warning(
            f"Zone center significantly offset from network center - possible coordinate system mismatch")
