...
def import_osm_network(osm_file_path: str, output_prefix: str = "data/grid") -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"Successfully converted OSM to SUMO network files with prefix: {output_prefix}")
        else:
...
class OSMImporter:
    ...
    def validate_osm_coverage(self) -> Dict[str, any]:
        ...
        try:
            ...
            lon_dist = (bbox['maxlon'] - bbox['minlon']) * 111000 * \
                      abs(bbox['minlat'] + bbox['maxlat']) / 2 * 0.017453  # account for latitude
            ...
            if len(highway_ways) < 10:
                validation_results['warnings'].append("Very few highway ways found - network may be incomplete")
            
            if len(traffic_signals) == 0:
                validation_results['warnings'].append("No traffic signals found - signals will be generated")
            
            if area_km2 > 5:
                validation_results['warnings'].append(f"Large area ({area_km2:.2f} km²) - simulation may be slow")
            ...
            if missing_types:
                validation_results['warnings'].append(f"Missing highway types: {missing_types}")
            
            logger.info(f"OSM validation complete: {len(highway_ways)} highway ways, {len(traffic_signals)} signals")
    ...
    def extract_osm_metadata(self) -> Dict[str, any]:
        """
        Extract metadata for simulation configuration
        
        Returns:
            dict: Metadata including bounds, road hierarchy, signal count
        """
        validation_results = self.validate_osm_coverage()
        
        if not validation_results['valid']:
            raise ValueError(f"Invalid OSM file: {validation_results.get('error', 'Unknown error')}")
        
        # Extract useful metadata
        metadata = {
            'source_file': str(self.osm_file_path),
            'bounding_box': validation_results['bounding_box'],
            'area_km2': validation_results['area_km2'],
            'network_stats': {
                'total_intersections': validation_results['total_nodes'],
                'total_roads': validation_results['highway_ways'],
                'traffic_signals': validation_results['traffic_signals']
            },
            'road_hierarchy': validation_results['highway_types'],
    ...
    def import_to_sumo_files(self, output_prefix: Optional[str] = None) -> Dict[str, Path]:
        ...
        logger.info(f"Converting OSM to SUMO files: {self.osm_file_path} -> {prefix}")
        ...
        if len(created_files) < 3:  # At least nod, edg, con should exist
            raise RuntimeError(f"Insufficient files created. Expected at least nod, edg, con files.")
        
        logger.info(f"Successfully converted OSM to SUMO files: {len(created_files)} files created")
    ...
    def get_network_statistics(self, sumo_files: Dict[str, Path]) -> Dict[str, any]:
                    ...
                    try:
                        ...
                        if shape:
                            ...
                            if len(coords) >= 2:
                                ...
                                stats['edge_lengths'].append(len(coords) * 50)  # ~50m per segment
                    except:
                        pass
        ...
        if stats['edge_lengths']:
            stats['avg_edge_length'] = sum(stats['edge_lengths']) / len(stats['edge_lengths'])
...
def import_osm_network(osm_file_path: str, output_dir: str = "data", 
    ...
    if not validation['valid']:
        raise ValueError(f"Invalid OSM file: {validation.get('error', 'Unknown error')}")
