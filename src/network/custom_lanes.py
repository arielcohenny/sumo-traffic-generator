...
def apply_custom_lane_configs(custom_lane_config: CustomLaneConfig) -> None:
    ...
    affected_junctions = _calculate_affected_junctions(custom_lane_config.edge_configs)
    ...
    print(f"Successfully applied custom lane configurations for {len(custom_lane_config.edge_configs)} edges")
...
def _update_edges_file(edg_root, custom_lane_config: CustomLaneConfig) -> None:
    ...
    for edge in edg_root.findall("edge"):
        ...
        base_edge_id = edge_id.replace("_H_s", "")  # Remove head segment suffix
...
def _generate_downstream_connections(con_root, edg_root, edge_id: str, movements: List[Dict],
    ...
    if edge_id in edge_coords:
        ...
        movement_to_head_lanes = assign_lanes_by_angle(movements_with_angles, head_lanes)
...
def _recreate_junction_traffic_light(tll_root, con_root, junction_id: str) -> None:
    ...
    if connection_count > 0:
        ...
        state_green_ns = "G" * (connection_count // 2) + "r" * (connection_count - connection_count // 2)
        state_yellow_ns = "y" * (connection_count // 2) + "r" * (connection_count - connection_count // 2)
        state_green_ew = "r" * (connection_count // 2) + "G" * (connection_count - connection_count // 2)
        state_yellow_ew = "r" * (connection_count // 2) + "y" * (connection_count - connection_count // 2)
