...
# Synthetic Grid Experiment Run 86
...
echo "🔄 Starting experiment run 86 (light load)"
...
try:
    result = subprocess.run([
        ...
        '--departure_pattern', 'rush_hours:7-9:40,17-19:30,rest:30',
        '--end-time', '7300',
    ...
    with open('evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/tree_method/simulation.log', 'w') as f:
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/tree_method/simulation.log" ]; then
        echo "   Last 3 lines of log:"
        tail -n 3 "evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/tree_method/simulation.log"
    fi
fi
...
try:
    ...
    # Write output to log file
    with open('evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/actuated/simulation.log', 'w') as f:
        f.write(result.stdout)
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/actuated/simulation.log" ]; then
        echo "   Last 3 lines of log:"
        tail -n 3 "evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/actuated/simulation.log"
    fi
fi
...
try:
    ...
    # Write output to log file
    with open('evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/fixed/simulation.log', 'w') as f:
        f.write(result.stdout)
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/fixed/simulation.log" ]; then
        echo "   Last 3 lines of log:"
        tail -n 3 "evaluation/benchmarks/synthetic-grids/grids-5x5/86/results/fixed/simulation.log"
    fi
fi

echo "🏁 Experiment run 86 completed"
