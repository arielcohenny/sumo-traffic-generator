...
# Synthetic Grid Experiment Run 195
...
echo "🔄 Starting experiment run 195 (moderate load)"
...
try:
    result = subprocess.run([
        ...
        '--block_size_m', '200',
        '--num_vehicles', '800',
        '--vehicle_types', 'passenger 50 commercial 30 public 20',
        '--routing_strategy', 'shortest 50 realtime 40 fastest 10',
        '--departure_pattern', 'rush_hours:7-9:40,17-19:30,rest:20',
        '--end-time', '7300',
    ...
    with open('evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/tree_method/simulation.log', 'w') as f:
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/tree_method/simulation.log" ]; then
        echo "   Last 3 lines of log:"
        tail -n 3 "evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/tree_method/simulation.log"
    fi
fi
...
try:
    ...
    # Write output to log file
    with open('evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/actuated/simulation.log', 'w') as f:
        f.write(result.stdout)
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/actuated/simulation.log" ]; then
        echo "   Last 3 lines of log:"
        tail -n 3 "evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/actuated/simulation.log"
    fi
fi
...
try:
    ...
    # Write output to log file
    with open('evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/fixed/simulation.log', 'w') as f:
        f.write(result.stdout)
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/fixed/simulation.log" ]; then
        echo "   Last 3 lines of log:"
        tail -n 3 "evaluation/benchmarks/synthetic-grids/grids-5x5/195/results/fixed/simulation.log"
    fi
fi

echo "🏁 Experiment run 195 completed"
