...
"""
Grid-Specific Experiment Runner for 7x7 Synthetic Grid
...
SEED=$((42 + {seed_offset}))
timeout {config['execution_settings']['timeout_minutes']}m \\
env PYTHONUNBUFFERED=1 python -m src.cli $BASE_PARAMS \\
--traffic_control {method} \\
--seed $SEED \\
> "evaluation/benchmarks/synthetic-grids/grids-7x7/{experiment['run_id']}/results/{method}/simulation.log" 2>&1

if [ $? -eq 0 ]; then
    echo "   ✅ {method} completed successfully"
else
    echo "   ❌ {method} failed or timed out"
fi
"""
...
def create_analyze_results_script(run_dir, experiment):
    ...
    if template_path.exists():
        ...
        adapted_content = template_content.replace(
            ...
            'Experiment1-realistic-high-load', f'grids-7x7'
        )
...
def main():
    ...
    print("SYNTHETIC GRID EXPERIMENT RUNNER - 7x7 GRID")
    print("=" * 80)
    ...
    print(f"\\n{'=' * 80}")
    print("🏁 7x7 GRID EXPERIMENTS COMPLETE")
    print(f"{'=' * 80}")
