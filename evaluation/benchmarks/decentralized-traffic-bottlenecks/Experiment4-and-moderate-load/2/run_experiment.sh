...
# Check if file contains valid XML first
if grep -q "404: Not Found" "$DATASET_PATH/simulation.sumocfg.xml"; then
    echo "Error: Dataset file contains '404: Not Found' - invalid dataset"
    echo "Dataset files for Experiments 2, 3, 4 need to be properly downloaded"
    exit 1
fi

END_TIME=$(grep -o 'end value="[0-9]*"' "$DATASET_PATH/simulation.sumocfg.xml" | grep -o '[0-9]*')
if [ -z "$END_TIME" ]; then
    echo "Error: Could not extract end time from simulation.sumocfg.xml"
    echo "File content:"
    head -5 "$DATASET_PATH/simulation.sumocfg.xml"
    exit 1
fi
