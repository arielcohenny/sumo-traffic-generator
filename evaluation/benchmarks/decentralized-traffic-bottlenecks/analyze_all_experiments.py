...
def load_all_experiment_results():
    ...
    for experiment in experiments:
        ...
        for run in range(1, 21):
            results_file = exp_dir / \
                str(run) / "results" / "analysis_results.json"
            ...
            if results_file.exists():
                try:
                    ...
                    print(
                        f"    Warning: Could not load {experiment} run {run}: {e}")
    ...
    for experiment in experiments:
        dataset_dir = benchmark_dir.parent.parent.parent / \
            'datasets' / 'decentralized_traffic_bottleneck' / experiment
            ...
            with open(arrived_file, 'r') as f:
                ...
                for i in range(1, min(21, len(lines))):  # Skip header, process runs 1-20
                    ...
                    if len(parts) >= 4:
                        ...
                        original_results['tree']['vehicles_arrived']['values'].append(
                            int(parts[1]))
                        original_results['fixed (uniform)']['vehicles_arrived']['values'].append(
                            int(parts[2]))
                        original_results['actuated']['vehicles_arrived']['values'].append(
                            int(parts[3]))
            ...
            with open(duration_file, 'r') as f:
                ...
                for i in range(1, min(21, len(lines))):  # Skip header, process runs 1-20
                    ...
                    if len(parts) >= 4:
                        ...
                        original_results['tree']['avg_duration']['values'].append(
                            int(parts[1]))
                        original_results['fixed (uniform)']['avg_duration']['values'].append(
                            int(parts[2]))
                        original_results['actuated']['avg_duration']['values'].append(
                            int(parts[3]))

    except Exception as e:
        print(
            f"    Warning: Could not load original dataset results from {dataset_dir}: {e}")
...
def calculate_method_statistics(method_data, metric):
    ...
    values = [run[metric]
              for run in method_data if run.get(metric) is not None]
    if not values:
        return None
...
def analyze_master_results():
    ...
    print(
        f"📊 Total results collected: {total_runs} (from {total_completed_runs} completed runs)")
    ...
    for experiment, summary in experiment_summaries.items():
        ...
        print(
            f"{experiment}: {runs_completed}/20 runs completed ({runs_completed/20*100:.0f}%)")
    print("")
            ...
            for metric in ['vehicles_entered', 'vehicles_arrived', 'avg_duration', 'completion_rate']:
                ...
                if stats:
                    if metric == 'completion_rate':
                        print(
                            f"  {metric.replace('_', ' ').title()}: {stats['mean']:.3%} ± {stats['std']:.3%}")
                        print(
                            f"    Range: {stats['min']:.3%} - {stats['max']:.3%}")
                    elif metric == 'avg_duration':
                        print(
                            f"  {metric.replace('_', ' ').title()}: {stats['mean']:.1f} ± {stats['std']:.1f} steps")
                        print(
                            f"    Range: {stats['min']:.1f} - {stats['max']:.1f} steps")
                    else:
                        print(
                            f"  {metric.replace('_', ' ').title()}: {stats['mean']:,.0f} ± {stats['std']:,.0f}")
                        print(
    ...
    for our_method, orig_method in comparison_mapping.items():
        if our_method in master_stats and orig_method in original_stats:
            ...
            if 'vehicles_arrived' in our_stats and 'vehicles_arrived' in orig_stats:
                ...
                print(
                    f"  Vehicles Arrived: {our_mean:,.0f} vs {orig_mean:,.0f} ({diff_pct:+.1f}%)")
    ...
    for method1, method2 in comparisons:
        if method1 in master_stats and method2 in master_stats:
            ...
            if 'vehicles_arrived' in master_stats[method1] and 'vehicles_arrived' in master_stats[method2]:
                ...
                print(
                    f"  Vehicles Arrived: {mean1:,.0f} vs {mean2:,.0f} ({improvement:+.1f}%)")
            ...
            if 'completion_rate' in master_stats[method1] and 'completion_rate' in master_stats[method2]:
                ...
                improvement = ((mean1 - mean2) / mean2) * 100
                print(
                    f"  Completion Rate: {mean1:.3%} vs {mean2:.3%} ({improvement:+.1f}%)")
    ...
    if 'tree' in master_stats:
        ...
        if 'actuated' in master_stats and 'fixed' in master_stats:
            ...
            if 'vehicles_arrived' in tree_stats and 'vehicles_arrived' in actuated_stats:
                tree_vs_actuated = ((tree_stats['vehicles_arrived']['mean'] - actuated_stats['vehicles_arrived']
                                    ['mean']) / actuated_stats['vehicles_arrived']['mean']) * 100
                print(f"Tree Method vs SUMO Actuated:")
                print(
                    f"  Vehicles Arrived Improvement: {tree_vs_actuated:+.1f}%")

            if 'vehicles_arrived' in tree_stats and 'vehicles_arrived' in fixed_stats:
                tree_vs_fixed = ((tree_stats['vehicles_arrived']['mean'] - fixed_stats['vehicles_arrived']
                                 ['mean']) / fixed_stats['vehicles_arrived']['mean']) * 100
            ...
            if 'completion_rate' in tree_stats and 'completion_rate' in actuated_stats:
                completion_improvement = (
                    tree_stats['completion_rate']['mean'] - actuated_stats['completion_rate']['mean']) * 100
                print(f"\nCompletion Rate Advantage:")
                print(
                    f"  vs Actuated: {completion_improvement:+.2f} percentage points")

            if 'completion_rate' in tree_stats and 'completion_rate' in fixed_stats:
                completion_improvement = (
                    tree_stats['completion_rate']['mean'] - fixed_stats['completion_rate']['mean']) * 100
                print(
                    f"  vs Fixed: {completion_improvement:+.2f} percentage points")
    ...
    if 'tree' in master_stats and 'actuated' in master_stats and 'fixed' in master_stats:
        ...
        if 'vehicles_arrived' in tree_data and 'vehicles_arrived' in actuated_data:
            validation_metrics['vs_actuated_vehicles_improvement_pct'] = float(
                ((tree_data['vehicles_arrived']['mean'] - actuated_data['vehicles_arrived']
                 ['mean']) / actuated_data['vehicles_arrived']['mean']) * 100
            )
        ...
        if 'vehicles_arrived' in tree_data and 'vehicles_arrived' in fixed_data:
            validation_metrics['vs_fixed_vehicles_improvement_pct'] = float(
                ((tree_data['vehicles_arrived']['mean'] - fixed_data['vehicles_arrived']
                 ['mean']) / fixed_data['vehicles_arrived']['mean']) * 100
            )
        ...
        if 'completion_rate' in tree_data and 'completion_rate' in actuated_data:
            validation_metrics['vs_actuated_completion_improvement_pts'] = float(
                (tree_data['completion_rate']['mean'] -
                 actuated_data['completion_rate']['mean']) * 100
            )
        ...
        if 'completion_rate' in tree_data and 'completion_rate' in fixed_data:
            validation_metrics['vs_fixed_completion_improvement_pts'] = float(
                (tree_data['completion_rate']['mean'] -
                 fixed_data['completion_rate']['mean']) * 100
            )
    ...
    print("=" * 40)
    print(
        f"Analyzed {total_completed_runs}/80 completed runs ({total_completed_runs/80*100:.1f}%)")
